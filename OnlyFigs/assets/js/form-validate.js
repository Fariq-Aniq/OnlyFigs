	...
	//}else if( f.phone.value == '' ){
		...
		notice( f.message );
		 ...
		 jQuery.ajax({
			url: 'mail.php',
			type: 'post',
			data: jQuery('form#OnlyFigs-contact').serialize(),
			complete: function(data) {
				jQuery('#form_status').html(data.responseText);
				jQuery('#OnlyFigs-contact').find('input,textarea').attr({value:''});
				jQuery('#OnlyFigs-contact').css({opacity:1});
				jQuery('#OnlyFigs-contact').remove();
			}
		});
		jQuery('#form_status').html('<span class="loading">Sending your message...</span>');
		jQuery('#OnlyFigs-contact').animate({opacity:0.3});
		jQuery('#OnlyFigs-contact').find('input,textarea,button').css('border','none').attr({'disabled':''});
	}
...
function notice( f ){
	jQuery('#OnlyFigs-contact').find('input,textarea').css('border','none');
